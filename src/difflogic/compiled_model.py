        ...
        for var_id, (gate_a, gate_b, gate_op) in enumerate(zip(layer_a, layer_b, layer_op)):
            if self.device == "cpu" and layer_id == len(prefix_sums) - 1:
                ...
                if layer_id == 0:
                    ...
                    b = f"v{prefix_sums[layer_id - 1] + gate_b}"
                code.append(
                    f"\tconst {BITS_TO_DTYPE[self.num_bits]} v{prefix_sums[layer_id] + var_id} = \
                    {self.get_gate_code(a, b, gate_op)};"
                )
        return code
...
void apply_logic_gate_net (bool const *inp, {BITS_TO_DTYPE[32]} *out, size_t len) {{
    ...
    {BITS_TO_DTYPE[self.num_bits]} *out_temp_o = malloc({log2_of_num_neurons_per_class_ll}* \
    sizeof({BITS_TO_DTYPE[self.num_bits]}));
    ...
    for(size_t i = 0; i < len; ++i) {{
        ...
        for(size_t d = 0; d < {self.num_inputs}; ++d) {{
            ...
            for(size_t b = 0; b < {self.num_bits}; ++b) {{
                res <<= 1;
                res += !!(inp[i * {self.num_inputs} * {self.num_bits} + ({self.num_bits} - b - 1) * \
                {self.num_inputs} + d]);
            }}
            inp_temp[d] = res;
        }}
        ...
        for(size_t c = 0; c < {self.num_classes}; ++c) {{  // for each class
            ...
            for(size_t a = 0; a < {self.layers[-1][0].shape[0] // self.num_classes}; ++a) {{
                {BITS_TO_DTYPE[self.num_bits]} carry = out_temp[c * \
                    {self.layers[-1][0].shape[0] // self.num_classes} + a];
...
""")
    ...
    def compile(self, opt_level=1, save_lib_path=None, verbose=False):
        ...
        with tempfile.NamedTemporaryFile(suffix=".so") as lib_file:
            with tempfile.NamedTemporaryFile(mode="w", suffix=".c" if self.device != "cuda" else ".cu") as c_file:
                if self.device == "cpu":
                ...
                if verbose:
                    print(
                        "C code created and has {} lines. (temp location {})".format(len(code.split("\n")), c_file.name)
                    )
